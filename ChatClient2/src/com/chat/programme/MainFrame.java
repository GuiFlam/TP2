...
public class MainFrame extends JFrame implements Runnable, Observateur {
    ...
    public static final String TITRE = "Echec et chat";
    ...
    public void seMettreAJour(Observable observable) {
        if (!client.isConnecte()) {
            mDemarrer.getItem(0).setVisible(true);
            mDemarrer.getItem(1).setVisible(false);
            this.setTitle(TITRE);
            this.panneauPrincipal.setVisible(false);
            panneauPrincipal.vider();
        }
        else {
            mDemarrer.getItem(0).setVisible(false);
            mDemarrer.getItem(1).setVisible(true);
            mDemarrer.getItem(1).setEnabled(true);
        }
    }
}